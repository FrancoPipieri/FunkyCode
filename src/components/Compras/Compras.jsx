...
function Compras(){
    ...
    return ( 
       ...
       <form className='flex flex-col items-center p-5' onSubmit={handleSubmit} >
            ...
            <button type='submit' className='button mx-2.5 my-auto w-auto'>Buscar</button>
        </form> : 
        ...
        {lastBuy?.compra.map((item)=>{
         ...
         <dl className='mb-3 border-double border-2'>
          <div className="bg-gray-100 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 ">{item.item.title}</dd>
            <dd><img src={item.item.filename} alt="" className='w-4/12 mx-auto' /></dd>
          </div>
          <div className="bg-slate-50 px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-xs font-medium text-gray-500">Series:</dt>
            <dd className="mt-1 text-xs text-gray-900 sm:mt-0 sm:col-span-2">{item.item.series}</dd>
          </div>
          <div className="bg-gray-100 px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-xs font-medium text-gray-500">Precio:</dt>
            <dd className="mt-1 text-xs text-gray-900 sm:mt-0 sm:col-span-2">${item.item.price}</dd>
          </div>
          <div className="bg-slate-50 px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt className="text-xs font-medium text-gray-500">Cantidad</dt>
            <dd className="mt-1 text-xs text-gray-900 sm:mt-0 sm:col-span-2">{item.quantity}</dd>
          </div>
          </dl>
          </>
          )
          ...
          <div className="bg-white px-4 py-3 items-center	 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 border-double border-2">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 font-bold sm:col-span">${lastBuy?.total}</dd>
            <button className='button' onClick={borrar}>Limpiar</button>
          </div>
